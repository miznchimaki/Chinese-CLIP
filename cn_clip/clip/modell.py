...
class VisualTransformer(nn.Module):
    def __init__(
        ...
        self.width = self.hidden_size = width
...
class CLIP(nn.Module):
    def __init__(
        ...
        # arguments for VTM (Vision Text Matching) loss
        vtm_loss: bool = False,
        vtm_loss_weight: float = 1.0,
        vtm_hard_sample_ratio: float = None,
        vtm_hard_sample_num: float = None
    ):
        super().__init__()
        ...
        # prepare for VTM (Vision Text Matching) loss training
        self.vtm_loss = vtm_loss
        self.vtm_loss_weight = vtm_loss_weight
        self.vtm_hard_sample_ratio = vtm_hard_sample_ratio
        self.vtm_hard_sample_num = vtm_hard_sample_num
        if self.vtm_loss:
            self.vtm_head = nn.Linear(
                in_features=self.bert.config.hidden_size + self.visual.hidden_size,
                out_features=2,
                bias=False
            )
        else:
