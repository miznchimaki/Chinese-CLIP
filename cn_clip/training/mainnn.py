...
def main():
    ...
    # VTM (Vision Text Matching) loss arguments
    model_info['vtm_loss'] = args.vtm_loss
    model_info['vtm_loss_weight'] = args.vtm_loss_weight
    model_info['vtm_hard_sample_ratio'] = args.vtm_hard_sample_ratio
    model_info['vtm_hard_sample_num'] = args.vtm_hard_sample_num
    if args.vtm_loss:
        if not (args.vtm_hard_sample_ratio and args.vtm_hard_sample_num):
            raise ValueError(
                'when using VTM (Vision Text Matching) loss to train Chinese-CLIP model, '
                'you should specify either `--vtm-hard-sample-ratio`, or `--vit-hard-sample-num` argument, '
                'but get both two None'
            )
        if args.vtm_hard_sample_ratio and args.vtm_hard_sample_num:
            warnings.warn(
                'value of `--vtm-hard-sample-ratio` will be covered by `--vtm-hard-sample-num`, '
                'hence set `--vtm-hard-sample-ratio` to None'
            )
            args.vtm_hard_sample_ratio = None
